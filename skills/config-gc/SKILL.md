...
description: Use when the user wants to clean up, audit, or slim down a Claude Code
  ...
  for redundant, stale, or orphaned items. Requires confirmation before removing anything.
origin: ECC
---

# Config GC — Garbage Collection for Claude Code Setups

Like runtime garbage collection: periodically scan for unreferenced, redundant, expired, or low-value items and reclaim the space. The key difference: **collection requires a human in the loop.**
...
4. **Soft-delete confirmed items**: `.disabled` rename for skills/hooks, `_gc_trash/<date>/` move for files. For JSON permission entries: back up the settings file, record each removed entry verbatim in `gc_log.md`, then drop it from the `allow` array with `jq`. Hard-delete only when explicitly asked.
